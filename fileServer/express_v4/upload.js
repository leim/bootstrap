            ...
            if (err) {
                ...
                try {
                    ...
                    if (config.keepFileName) {
                        filePath = path.join('upload', dateStr, name);  //文件名保留上传的名称
                    } else {
                        filePath = path.join('upload', dateStr, rack() + path.extname(name).toLowerCase());   //文件名选择随机数
                    }
                    var destPath = path.join(staticPath, filePath);
                    fs.move(tmpPath, destPath, {clobber: config.overwrite}, function (err) {
                        if (err) {
                            //handle error
                            console.log('upload file excption: ' + err.message);
                            result(res, {statusCode: 905, message: err.message});
